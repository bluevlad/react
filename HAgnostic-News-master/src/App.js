                  ...
                  <View style={styles.itemRow}>
                    <Text style={{ flex: 1 }}>
                      <Text
                        ...
                        onPress={() => onOpenUrl(`https://news.ycombinator.com/item?id=${item.objectID}`)}>{i + 1}. {item.title}</Text>
                      <Text style={[{ flex: 1, color: '#979797' }, cursorStyle]}> {item.author}</Text>
                    </Text>
                  </View>
                  <View style={styles.itemSubRow}>
                    <Text style={{ padding: 2 }}>{item.story_id} points </Text>
                    <Text style={{ padding: 2 }}> by {item.matchLevel}</Text>
                    <Text style={{ padding: 2 }}>| {item.value} c. |</Text>
                    <Text
                      onPress={() => onOpenUrl(`https://news.ycombinator.com/item?id=${item.objectID}`)}
                      style={[{ padding: 2, flex: 1, textDecorationLine: 'underline' }, cursorStyle]}> {item.created_at}</Text>
                  </View>
                </View>
