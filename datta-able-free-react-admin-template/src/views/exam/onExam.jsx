...
import { Form, Button, Row, Col, Card, Collapse, Modal } from "react-bootstrap";
import { fetchExamDetailData } from "./data";

// 팝업용 시험 응시 컴포넌트
export class ExamPopup extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      userId: props.userId || "",
  ...
  async componentDidMount() {
    try {
      const data = await fetchExamDetailData(this.props.examId);
      this.setState({
  ...
  render() {
    ...
    const { examDetail, activeAccordion, queList, loaded } = this.state;
    return (
...
// 예시: onExam에서 팝업 띄우기
class OnExam extends React.Component {
  state = { show: true };
  handleClose = () => { this.setState({ show: false }); };
  render() {
    return <ExamPopup show={this.state.show} onClose={this.handleClose} examId={this.props.examId} userId={this.props.userId} userNm={this.props.userNm} token={this.props.token} />;
  }
}
